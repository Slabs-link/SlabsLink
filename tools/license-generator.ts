...
class LicenseGenerator {
  ...
  constructor() {
    ...
    this.pool = new Pool({
      ...
      password: process.env.DB_PASSWORD || 'M4tumb4!'
    });
  }
  ...
  async createLicense(
    expirationDate: Date,
    whatsappIntegration: boolean = false,
    googleCalendarIntegration: boolean = false,
    whatsappConfig: { apiPath: string } | null = null,
    calendarConfig: { apiPath: string } | null = null
  ): Promise<License> {
    ...
    const features = {
      ...
      whatsappConfig: whatsappIntegration ? whatsappConfig : null,
      calendarConfig: googleCalendarIntegration ? calendarConfig : null
    };
  ...
  async ensureLicensesTableExists(): Promise<void> {
  ...
  async ensureDatabase(): Promise<void> {
    try {
      ...
      const checkDbQuery = `
        SELECT 1 FROM pg_database WHERE datname = 'slabslink'
      `;
      const result = await this.pool.query(checkDbQuery);
      
      if (result.rows.length === 0) {
        console.log('Database "slabslink" does not exist. Creating...');
        
        // Create the database
        await this.pool.query(`CREATE DATABASE slabslink`);
        console.log('Database "slabslink" created successfully');
        ...
        // If database exists, reconnect to it
        await this.pool.end();
        this.pool = new Pool({
          host: process.env.DB_HOST || 'localhost',
          port: parseInt(process.env.DB_PORT || '5432'),
...
async function runCLI() {
      ...
      rl.question('Enter expiration date (YYYY-MM-DD): ', async (dateStr) => {
        ...
        rl.question('Include WhatsApp integration? (y/n): ', async (whatsappAnswer) => {
          ...
          rl.question('Include Google Calendar integration? (y/n): ', async (googleAnswer) => {
            ...
            try {
              const license = await generator.createLicense(
                expirationDate,
                whatsappIntegration,
                googleCalendarIntegration
              );
  ...
  // Remove the duplicate close method that was here
}
