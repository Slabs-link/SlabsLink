...
const NotificationTemplateEditor: React.FC = () => {
  ...
  const handleDragEnd = (result: DropResult) => {
    ...
    // Se il campo viene trascinato nell'area del template
    if (source.droppableId === 'availableFields' && destination.droppableId === 'templateContent') {
      const field = availableFields[source.index];
      const textField = document.getElementById('template-content') as HTMLTextAreaElement;
      const cursorPosition = textField?.selectionStart || templateContent.length;
      
      // Inserisci la variabile nella posizione del cursore
      const newContent = 
  ...
  const insertField = (field: FieldOption) => {
    ...
    setTimeout(() => {
      textField.focus();
      textField.selectionStart = cursorPosition + field.variable.length;
      textField.selectionEnd = cursorPosition + field.variable.length;
    }, 0);
  };
  ...
  return (
    <Box sx={{ display: 'flex', bgcolor: '#fafafa', minHeight: '100vh' }}>
      ...
      <Box sx={{ flexGrow: 1, p: 3 }}>
        <SectionTitle variant="h4">Editor Template di Notifica</SectionTitle>
          ...
          <Grid item xs={12} md={8}>
            <ContentCard>
              ...
              <TextField
                ...
                rows={8}
                required
                placeholder="Scrivi qui il contenuto del template o trascina i campi disponibili"
              />
              
              <TemplatePreview>
                <Typography variant="subtitle2" gutterBottom>Anteprima</Typography>
                <Typography variant="body2">
                  {templateContent.split(/{{([^}]+)}}/g).map((part, index) => {
                    // Se l'indice è dispari, è una variabile
                    return index % 2 === 1 ? 
                      <Chip key={index} size="small" label={part} color="primary" variant="outlined" /> : 
                      part;
                  })}
                </Typography>
              </TemplatePreview>
